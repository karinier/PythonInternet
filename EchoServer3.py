...
def handle_request(request):
    ...
    if words[0].decode("utf-8") == 'GET':
        print("HTTP/1.1 200 OK GET request!")
        x=1
    elif words[0].decode("utf-8") == 'HEAD':
        print("HTTP/1.1 200 OK HEAD request!")
    elif words[0].decode("utf-8") == 'PUT':
        print("HTTP/1.1 200 OK put")
    else:
	    print("reformat the command!")
...
def client_talk(client_sock, client_addr):
    ...
    while data:
        ...
        if words[0].decode("utf-8") == 'GET':
            if words[1].decode("utf-8") == '/usr/Python/Assign6':
                print("HTTP/1.1 200 OK GET request!")
       #    stub=ArgumentParser.data[0]
       # if stub=='GET' : print('In the get loop.')
        handle_request(data)
        data = client_sock.recv(BUFSIZE)
